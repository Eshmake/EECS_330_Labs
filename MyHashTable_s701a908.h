...
#include "MyLinkedList_s701a908.h"
...
{ 
  private:
    ...
    void preCalPrimes(const size_t n)
    {
        ...
        size_t limit = std::max(2*n, size_t(100)); 
        MyVector<bool> is_prime(limit + 1);
    ...
    void rehash(const size_t new_size)
    {
        ...
        size_t oldSize = theSize;
            ...
            MyVector<MyLinkedList<HashedObj<KeyType, ValueType>>*> temp_hash_table(new_size);

            for(size_t i = 0; i < temp_hash_table.size(); ++i){
                temp_hash_table[i] = new MyLinkedList<HashedObj<KeyType, ValueType>>;
            }

            std::swap(hash_table, temp_hash_table);

            for(size_t i = 0; i < temp_hash_table.size(); ++i){
                for(auto itr = temp_hash_table[i]->begin(); itr != temp_hash_table[i]->end(); ++itr){
                    insert(std::move(*itr));
                }
            }
  ...
  public:
    ...
    {
        // code begins
        if(contains(x.key)) return false;
        ...
        if(2 * theSize >= hash_table.size()) doubleTable();
        ...
        HashFunc<KeyType> hash_func;
            ...
            for(auto itr = hash_table[bucket]->begin(); itr != hash_table[bucket]->end(); ++itr){
                if((*itr).key == key){
                    ...
                    hash_table[bucket]->erase(itr);
                    --theSize;

                    if(8 * theSize < hash_table.size()) halveTable();
                    return true;
                }
            }
