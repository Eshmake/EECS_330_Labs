...
class MyBST
{
  private:
    ...
    void rotateLeft(BinaryNode*& t)     //
    {
        ...
        tl->height = 1 +(height(tl->left) > height(tl->right) ? height(tl->left) : height(tl->right));
    ...
    void remove(const ComparableType& x, BinaryNode*& t)    //
    {
        ...
        if(x < t->element){
            ...
            BinaryNode* oldNode = t;    //store memory address of current node in new pointer

            t = (t->left != nullptr) ? t->left : t->right;  //updates node pointer to child if has one (or nullptr otherwise)
            delete oldNode;
